//HW к методичке 5: Удалить комментарии из CPP файла - текст между // и // или /* и  */
...
int main()
{
	...
	source.open(infile);//инициализация потоков переданным строкой адресом (или (infile.c_str()))
	target.open(outfile); //(или outfile.c_str() - функция c_str() возвращает указатель на массив содержащий текущую подстроку 
